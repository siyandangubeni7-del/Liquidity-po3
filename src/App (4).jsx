...
function ResultCard({ result }) {
  const isNeutral = (result.direction || '').toLowerCase() === 'neutral';
    ...
    <section className="result-card">
      ...
      <div className="direction-row">
        <div className={`direction ${isNeutral ? 'neutral' : isBuy ? 'buy' : 'sell'}`}>
          {isNeutral ? '−' : isBuy ? '↗' : '↘'} {(result.direction || '—').toUpperCase()}
        </div>
...
export default function App() {
  ...
  const handleHtfFile = useCallback((f) => {
    if (!f) return;
    setHtfFile(f);
    setResult(null);
    setError(null);
    setHtfPreview(URL.createObjectURL(f));
  }, []);

  const onHtfDrop = (e) => {
    e.preventDefault();
    const f = e.dataTransfer.files?.[0];
    if (f) handleHtfFile(f);
  };
  ...
  const scan = async () => {
    ...
    try {
      ...
      const body = { image: base64, mimeType: file.type || 'image/jpeg' };
      if (htfFile) {
        body.htfImage = await fileToBase64(htfFile);
        body.htfMimeType = htfFile.type || 'image/jpeg';
      }
      ...
      const isNeutralResult = (data.direction || '').toLowerCase() === 'neutral';
      const filter = isNeutralResult ? null : evaluateFilter(data);
      const entry = {
        ...
        pass: isNeutralResult ? null : filter.pass,
        outcome: isNeutralResult ? 'neutral' : 'pending',
      };
      setHistory((prev) => {
      ...
      {tab === 'scan' ? (
        <main className="main">
          ...
          <div className="upload-label-row">
            <span className="cell-label">ENTRY CHART {showHtf && '(LTF)'}</span>
          </div>
          <section
            className="dropzone"
          ...
          {!showHtf ? (
            <button className="add-htf-btn" onClick={() => setShowHtf(true)}>
              + Add higher timeframe chart (recommended)
            </button>
          ) : (
            <>
              <div className="upload-label-row">
                <span className="cell-label">HIGHER TIMEFRAME (HTF) — MACRO CONTEXT</span>
                <button className="remove-htf-btn" onClick={() => { setShowHtf(false); clearHtf(); }}>Remove</button>
              </div>
              <section
                className="dropzone dropzone-htf"
                onDragOver={(e) => e.preventDefault()}
                onDrop={onHtfDrop}
                onClick={() => htfInputRef.current?.click()}
              >
                <input
                  ref={htfInputRef}
                  type="file"
                  accept="image/*"
                  hidden
                  onChange={(e) => handleHtfFile(e.target.files?.[0])}
                />
                {htfPreview ? (
                  <img src={htfPreview} alt="HTF chart preview" className="preview" />
                ) : (
                  <div className="dropzone-empty">
                    <div className="dz-icon">⌁</div>
                    <p>Upload the H1/H4 (or higher) chart</p>
                    <p className="dz-sub">Used for macro bias, not entry</p>
                  </div>
                )}
              </section>
            </>
